...
| §7.1 | L264 | The user MUST explicitly select the active package. Firmware MUST NOT infer or | referenced | full_set_in_order → (file)<br>storage_sets → measured_user_data_tracks_set_files<br>web_api_core → route_parsing<br>web_api_repository_handlers → set_delete_and_persistent_readback |
...
| §10.7 | L628 | They MUST NOT be duplicated as inconsistent magic numbers. | referenced | storage_macros → missing_set_and_revision_overflow<br>web_api_admin_boundary → (file)<br>web_api_admin_boundary → backup_failure_without_detail_stays_plain |
...
| §13.2 | L844 | Firmware MUST NOT automatically format either filesystem. | referenced | web_server_adapter_json_static → static_file_selection<br>check-layer-boundaries.sh (gate script)<br>check-mount-policy.sh (gate script) |
| §13.2 | L847 | Web-assets failure MUST NOT expose an unauthenticated fallback UI. | referenced | web_server_adapter_json_static → static_file_selection<br>check-layer-boundaries.sh (gate script)<br>check-mount-policy.sh (gate script) |
| §13.3 | L876 | This is the whole tree. There MUST NOT be a per-package directory, a per-object file, | referenced | storage_mount → unmount_continues_after_one_failure<br>storage_package_restore → (file)<br>storage_sets → (file)<br>storage_sets → duplicate_index_is_discarded_and_output_cleared<br>web_api_repository_handlers → (file) |
| §13.5 | L920 | required the opposite: that restore is "not atomic across sets, and MUST NOT | referenced | backup_restore → (file)<br>storage_package_restore → (file)<br>web_api_admin_boundary → restore_failure_is_visible |
| §13.5 | L929 | MUST NOT leave a mixture of old and new packages, and MUST NOT leave the device | referenced | backup_restore → (file)<br>storage_package_restore → (file)<br>web_api_admin_boundary → restore_failure_is_visible |
| §13.5 | L954 | real data — in favour of an older copy. Firmware MUST NOT decide the direction by | referenced | backup_restore → (file)<br>storage_package_restore → (file)<br>web_api_admin_boundary → restore_failure_is_visible |
| §13.5 | L961 | Restore MUST NOT perform the whole rewrite synchronously on the HTTP server task. | referenced | backup_restore → (file)<br>storage_package_restore → (file)<br>web_api_admin_boundary → restore_failure_is_visible |
| §13.6 | L988 | Deleting a corrupt file MUST NOT be reported as successful recovery. | referenced | management-screens → shows live redacted storage data<br>storage_macros → oversized_set_file_is_refused<br>storage_macros → set_local_crud_duplicate_and_order<br>web_api_admin_boundary → storage_snapshot_publishes_remaining_space<br>web_api_core → route_parsing |
| §13.7 | L997 | resource metadata. The server MUST NOT silently overwrite a newer edit. | referenced | web_api_repository_handlers → session_json_redaction |
| §14 | L1015 | The administrator password MUST NOT be stored in plaintext, nor in any form from | referenced | provisioning → corrupt_persisted_records<br>provisioning → load_error_and_uninitialized_calls<br>provisioning → no_stored_network_is_the_initial_state<br>provisioning → oversized_credentials_are_refused_without_side_effects<br>provisioning → station_credentials_survive_a_power_cycle<br>provisioning → storing_a_network_replaces_the_previous_one |
| §14 | L1023 | them is confinement rather than hashing: firmware MUST NOT emit either | referenced | provisioning → corrupt_persisted_records<br>provisioning → load_error_and_uninitialized_calls<br>provisioning → no_stored_network_is_the_initial_state<br>provisioning → oversized_credentials_are_refused_without_side_effects<br>provisioning → station_credentials_survive_a_power_cycle<br>provisioning → storing_a_network_replaces_the_previous_one |
| §14 | L1025 | report. A caller that needs an SSID MUST NOT be handed a copy of the whole | referenced | provisioning → corrupt_persisted_records<br>provisioning → load_error_and_uninitialized_calls<br>provisioning → no_stored_network_is_the_initial_state<br>provisioning → oversized_credentials_are_refused_without_side_effects<br>provisioning → station_credentials_survive_a_power_cycle<br>provisioning → storing_a_network_replaces_the_previous_one |
| §15.1 | L1055 | AP startup failure is a visible fatal network state. The firmware MUST NOT | referenced | app_core → normal_failure_matrix<br>wifi_ap → events_and_client_saturation |
| §15.2 | L1067 | firmware MUST NOT keep a list, and MUST NOT scan for, rank, or join any network | referenced | provisioning → load_error_and_uninitialized_calls<br>provisioning → no_stored_network_is_the_initial_state<br>provisioning → storing_a_network_disturbs_nothing_else<br>provisioning → storing_a_network_replaces_the_previous_one<br>web_setup → success_requires_code_and_confirmation |
| §15.2 | L1079 | availability a fatal-if-absent property, so it MUST NOT be made to wait on, or | referenced | provisioning → load_error_and_uninitialized_calls<br>provisioning → no_stored_network_is_the_initial_state<br>provisioning → storing_a_network_disturbs_nothing_else<br>provisioning → storing_a_network_replaces_the_previous_one<br>web_setup → success_requires_code_and_confirmation |
| §15.2 | L1084 | ignored: the device continues as access-point only. Firmware MUST NOT treat it | referenced | provisioning → load_error_and_uninitialized_calls<br>provisioning → no_stored_network_is_the_initial_state<br>provisioning → storing_a_network_disturbs_nothing_else<br>provisioning → storing_a_network_replaces_the_previous_one<br>web_setup → success_requires_code_and_confirmation |
| §15.2 | L1085 | as a startup failure, MUST NOT retry it in a way that delays or blocks the rest | referenced | provisioning → load_error_and_uninitialized_calls<br>provisioning → no_stored_network_is_the_initial_state<br>provisioning → storing_a_network_disturbs_nothing_else<br>provisioning → storing_a_network_replaces_the_previous_one<br>web_setup → success_requires_code_and_confirmation |
| §15.2 | L1086 | of startup, and MUST NOT discard the stored credentials because one join | referenced | provisioning → load_error_and_uninitialized_calls<br>provisioning → no_stored_network_is_the_initial_state<br>provisioning → storing_a_network_disturbs_nothing_else<br>provisioning → storing_a_network_replaces_the_previous_one<br>web_setup → success_requires_code_and_confirmation |
| §16.5 | L1166 | The console MUST NOT expose credentials or secret material even so, because | referenced | cancellation → (file)<br>check-credential-logging.sh (gate script) |
| §16.6 | L1195 | forgets a password MUST NOT have to choose between recovering the device and | referenced | provisioning → corrupt_persisted_records |
| §17 | L1317 | `GET /api/v1/backup` MUST NOT let one damaged object make the repository | referenced | backup_restore → (file)<br>storage_package → valid_set_and_backup_packages<br>storage_package_backup → backup_output_passes_secret_sentinel_scanner<br>storage_package_backup → uncompilable_macro_is_skipped_not_fatal |
| §17 | L1337 | MUST NOT report `200` for a run that failed to write some of them. | referenced | backup_restore → (file)<br>storage_package → valid_set_and_backup_packages<br>storage_package_backup → backup_output_passes_secret_sentinel_scanner<br>storage_package_backup → uncompilable_macro_is_skipped_not_fatal |
| §19 | L1399 | The device MUST NOT require any button, and MUST NOT require hardware to be | referenced | device_controls → runtime_failures |
| §20.1 | L1443 | swallow an `esp_err_t` | **UNMAPPED** | — |
| §20.1 | L1444 | cast away or discard an error result | **UNMAPPED** | — |
| §20.1 | L1445 | return success after partial completion | **UNMAPPED** | — |
| §20.1 | L1446 | log an error and then continue in an invalid state | **UNMAPPED** | — |
| §20.1 | L1447 | substitute empty data after parse failure | **UNMAPPED** | — |
| §20.1 | L1448 | silently retry forever | **UNMAPPED** | — |
| §20.1 | L1449 | silently downgrade authentication, storage, USB, or validation behavior | **UNMAPPED** | — |
| §20.1 | L1450 | use a dangerous fallback merely to keep the application running | **UNMAPPED** | — |
| §21.1 | L1492 | The defect MUST be fixed at its source. It MUST NOT be hidden, suppressed, | gate-enforced | check-static-analysis-policy.sh (gate script) |
| §21.3 | L1526 | The project MUST NOT modify ESP-IDF, managed components, npm dependencies, or | gate-enforced | check-static-analysis-policy.sh (gate script) |
| §21.4 | L1536 | First-party source and project configuration MUST NOT use warning suppression as | gate-enforced | check-static-analysis-policy.sh (gate script) |
| §26 | L1818 | Deferred features MUST NOT be partially or silently enabled in version 0.1. | gate-enforced | check-removed-features.sh (gate script) |
| §27 | L1841 | MUST NOT be assumed to exist. Implement the pages from this specification until | **UNMAPPED** | — |
...
| §10.7 | L627 | Limits MUST be centralized, visible through the API, and tested at boundaries. | referenced | storage_macros → missing_set_and_revision_overflow<br>web_api_admin_boundary → (file)<br>web_api_admin_boundary → backup_failure_without_detail_stays_plain |
| §10.7 | L632 | MB against a 512 KiB partition. Firmware MUST enforce the storage limits by | referenced | storage_macros → missing_set_and_revision_overflow<br>web_api_admin_boundary → (file)<br>web_api_admin_boundary → backup_failure_without_detail_stays_plain |
| §10.7 | L634 | count limits, and MUST reject an over-budget write with `507` (§17) rather than | referenced | storage_macros → missing_set_and_revision_overflow<br>web_api_admin_boundary → (file)<br>web_api_admin_boundary → backup_failure_without_detail_stays_plain |
...
| §12 | L726 | revision number | referenced | acceptance_reset → (file)<br>storage_object_json → macro_rejects_noncanonical_json |
...
| §12.2 | L783 | A macro's `source` MUST compile before it is stored. Creation and update | referenced | storage_object_json → set_document_rejects_duplicate_macro_ids<br>storage_object_json → set_document_round_trip |
...
| §13.3 | L851 | The `userdata` partition is **512 KiB**. The layout MUST be flat: one index file | referenced | storage_mount → unmount_continues_after_one_failure<br>storage_package_restore → (file)<br>storage_sets → (file)<br>storage_sets → duplicate_index_is_discarded_and_output_cleared<br>web_api_repository_handlers → (file) |
...
| §13.5 | L927 | it MUST be atomic: it either replaces the repository completely or leaves the | referenced | backup_restore → (file)<br>storage_package_restore → (file)<br>web_api_admin_boundary → restore_failure_is_visible |
| §13.5 | L928 | previous one in place. A failed or interrupted upload MUST be rolled back. It | referenced | backup_restore → (file)<br>storage_package_restore → (file)<br>web_api_admin_boundary → restore_failure_is_visible |
| §13.5 | L950 | The marker MUST carry which operation was in flight, not merely that one was. | referenced | backup_restore → (file)<br>storage_package_restore → (file)<br>web_api_admin_boundary → restore_failure_is_visible |
| §13.5 | L958 | to be atomic. It MUST be repeatable, so that an interrupted synchronization is | referenced | backup_restore → (file)<br>storage_package_restore → (file)<br>web_api_admin_boundary → restore_failure_is_visible |
| §13.6 | L987 | The error MUST name the object and MUST be surfaced through the API and the UI. | referenced | management-screens → shows live redacted storage data<br>storage_macros → oversized_set_file_is_refused<br>storage_macros → set_local_crud_duplicate_and_order<br>web_api_admin_boundary → storage_snapshot_publishes_remaining_space<br>web_api_core → route_parsing |
| §14 | L1030 | A stored record whose length does not match the current layout MUST be rejected | referenced | provisioning → corrupt_persisted_records<br>provisioning → load_error_and_uninitialized_calls<br>provisioning → no_stored_network_is_the_initial_state<br>provisioning → oversized_credentials_are_refused_without_side_effects<br>provisioning → station_credentials_survive_a_power_cycle<br>provisioning → storing_a_network_replaces_the_previous_one |
| §15.2 | L1083 | A station join that fails, times out, or is refused MUST be logged and otherwise | referenced | provisioning → load_error_and_uninitialized_calls<br>provisioning → no_stored_network_is_the_initial_state<br>provisioning → storing_a_network_disturbs_nothing_else<br>provisioning → storing_a_network_replaces_the_previous_one<br>web_setup → success_requires_code_and_confirmation |
| §16.2 | L1104 | issued on successful login, and it MUST be `HttpOnly` and `SameSite=Strict`. | referenced | network_security → (file)<br>web_request_policy → success_and_generated_request_id |
| §16.3 | L1128 | policy. The implementation MUST avoid unbounded per-IP state. | referenced | auth_additional_rate_tests → (file) |
| §16.4 | L1134 | route-specific body limits | referenced | web_request_policy → failure_statuses |
| §16.4 | L1135 | header count and size limits where configurable | referenced | web_request_policy → failure_statuses |
| §16.4 | L1136 | JSON nesting and collection limits | referenced | web_request_policy → failure_statuses |
| §16.4 | L1137 | bounded parsing memory | referenced | web_request_policy → failure_statuses |
| §16.4 | L1138 | request timeout | referenced | web_request_policy → failure_statuses |
| §16.4 | L1139 | upload size limit | referenced | web_request_policy → failure_statuses |
| §16.4 | L1140 | filename and ID validation | referenced | web_request_policy → failure_statuses |
| §16.4 | L1141 | correct content type | referenced | web_request_policy → failure_statuses |
| §16.5 | L1151 | the device's own SoftAP or, in development builds, a joined network - MUST | referenced | cancellation → (file)<br>check-credential-logging.sh (gate script) |
| §16.5 | L1152 | carry a valid RAM-only session cookie (§16.2). Authentication failures MUST be | referenced | cancellation → (file)<br>check-credential-logging.sh (gate script) |
| §16.5 | L1172 | third parties it MUST be excluded from the shipped image, since a shipped | referenced | cancellation → (file)<br>check-credential-logging.sh (gate script) |
| §16.6 | L1187 | It MUST clear the administrator password verifier and its salt, and the AP SSID | referenced | provisioning → corrupt_persisted_records |
| §16.6 | L1188 | and passphrase, and it MUST mark the device unprovisioned so first-run setup | referenced | provisioning → corrupt_persisted_records |
| §16.6 | L1191 | It MUST preserve everything the user did not lose: the device name, the settings | referenced | provisioning → corrupt_persisted_records |
| §16.6 | L1198 | Each reset increments a credential version, so a device MUST refuse the | referenced | provisioning → corrupt_persisted_records |
| §17 | L1314 | it, but external behavior and resource boundaries MUST remain equivalent and be | referenced | backup_restore → (file)<br>storage_package → valid_set_and_backup_packages<br>storage_package_backup → backup_output_passes_secret_sentinel_scanner<br>storage_package_backup → uncompilable_macro_is_skipped_not_fatal |
| §17 | L1322 | A partial backup MUST be self-describing, so it can never be mistaken for a | referenced | backup_restore → (file)<br>storage_package → valid_set_and_backup_packages<br>storage_package_backup → backup_output_passes_secret_sentinel_scanner<br>storage_package_backup → uncompilable_macro_is_skipped_not_fatal |
| §17 | L1330 | I/O, storage unavailable, timeout) MUST still fail the export, because | referenced | backup_restore → (file)<br>storage_package → valid_set_and_backup_packages<br>storage_package_backup → backup_output_passes_secret_sentinel_scanner<br>storage_package_backup → uncompilable_macro_is_skipped_not_fatal |
| §17 | L1336 | partial success MUST enumerate which packages were restored and which were not; it | referenced | backup_restore → (file)<br>storage_package → valid_set_and_backup_packages<br>storage_package_backup → backup_output_passes_secret_sentinel_scanner<br>storage_package_backup → uncompilable_macro_is_skipped_not_fatal |
| §19 | L1401 | product. GPIO assignment for the one remaining output MUST be configurable | referenced | device_controls → runtime_failures |
| §19 | L1417 | Cancellation MUST remain available during execution and delay actions, over | referenced | device_controls → runtime_failures |
| §19 | L1430 | Indicator semantics MUST be documented and testable. Failure LEDs do not replace | referenced | device_controls → runtime_failures |
| §20.1 | L1437 | Every operation MUST return, log, or expose an explicit success or failure. | **UNMAPPED** | — |
| §20.2 | L1456 | use component tags | referenced | app_core → residual_ownership_queries_trigger_cleanup |
| §20.2 | L1457 | identify state transitions | referenced | app_core → residual_ownership_queries_trigger_cleanup |
| §20.2 | L1458 | include object or execution IDs where safe | referenced | app_core → residual_ownership_queries_trigger_cleanup |
| §20.2 | L1459 | avoid passwords, tokens, raw cookie values, setup codes, and macro text that may contain secrets | referenced | app_core → residual_ownership_queries_trigger_cleanup |
| §20.2 | L1461 | use bounded formatting | referenced | app_core → residual_ownership_queries_trigger_cleanup |
| §20.2 | L1462 | distinguish user error, recoverable system error, and fatal invariant failure | referenced | app_core → residual_ownership_queries_trigger_cleanup |
| §20.3 | L1482 | A downloadable diagnostic report MUST redact secrets and macro source by | referenced | storage_atomic_recovery → stray_temporary_is_removed_at_boot<br>web_api_admin_boundary → storage_snapshot_publishes_remaining_space |
| §21.3 | L1512 | The quality gate MUST exclude: | gate-enforced | check-static-analysis-policy.sh (gate script) |
| §21.3 | L1529 | If a diagnostic originates exclusively in a third-party header, the tool MUST be | gate-enforced | check-static-analysis-policy.sh (gate script) |
| §21.5 | L1592 | MUST run the authoritative local quality gate. CI MUST call the same command. | gate-enforced | check-all.sh (gate script) |
| §21.5 | L1593 | The script MUST fail on the first failed phase or aggregate failures while still | gate-enforced | check-all.sh (gate script) |
| §21.5 | L1594 | returning nonzero; it MUST never mask failures. | gate-enforced | check-all.sh (gate script) |
| §23 | L1640 | The firmware build MUST fail when the expected web assets are absent, stale | gate-enforced | build-webfs-image.sh (gate script)<br>check-release-budgets.sh (gate script)<br>check-scripts.sh (gate script)<br>generate-flash-manifest.sh (gate script) |
| §23 | L1645 | Git commit | gate-enforced | build-webfs-image.sh (gate script)<br>check-release-budgets.sh (gate script)<br>check-scripts.sh (gate script)<br>generate-flash-manifest.sh (gate script) |
| §23 | L1646 | dirty/clean state | gate-enforced | build-webfs-image.sh (gate script)<br>check-release-budgets.sh (gate script)<br>check-scripts.sh (gate script)<br>generate-flash-manifest.sh (gate script) |
| §23 | L1647 | ESP-IDF version | gate-enforced | build-webfs-image.sh (gate script)<br>check-release-budgets.sh (gate script)<br>check-scripts.sh (gate script)<br>generate-flash-manifest.sh (gate script) |
| §23 | L1648 | managed-component lock hash | gate-enforced | build-webfs-image.sh (gate script)<br>check-release-budgets.sh (gate script)<br>check-scripts.sh (gate script)<br>generate-flash-manifest.sh (gate script) |
| §23 | L1649 | frontend lockfile hash | gate-enforced | build-webfs-image.sh (gate script)<br>check-release-budgets.sh (gate script)<br>check-scripts.sh (gate script)<br>generate-flash-manifest.sh (gate script) |
| §23 | L1650 | build type | gate-enforced | build-webfs-image.sh (gate script)<br>check-release-budgets.sh (gate script)<br>check-scripts.sh (gate script)<br>generate-flash-manifest.sh (gate script) |
| §23 | L1651 | build timestamp where reproducibility policy permits | gate-enforced | build-webfs-image.sh (gate script)<br>check-release-budgets.sh (gate script)<br>check-scripts.sh (gate script)<br>generate-flash-manifest.sh (gate script) |
| §23 | L1653 | Release builds MUST be reproducible from committed sources and lockfiles. | gate-enforced | build-webfs-image.sh (gate script)<br>check-release-budgets.sh (gate script)<br>check-scripts.sh (gate script)<br>generate-flash-manifest.sh (gate script) |
| §24.1 | L1661 | every supported ASCII character | referenced | macro_parser → fuzz_corpus |
| §24.1 | L1662 | shifted punctuation | referenced | macro_parser → fuzz_corpus |
| §24.1 | L1663 | every named key | referenced | macro_parser → printable_ascii |
| §24.1 | L1664 | every allowed modifier combination | referenced | macro_parser → delay_boundaries |
| §24.1 | L1665 | brace escaping | referenced | macro_parser → error_locations_and_directive_boundaries |
| §24.1 | L1666 | newline and tab normalization | referenced | macro_parser → named_keys_and_modifiers |
| §24.1 | L1667 | unknown directives | referenced | macro_parser → case_whitespace_and_line_endings |
| §24.1 | L1668 | malformed chords | referenced | macro_parser → delay_boundaries |
| §24.1 | L1669 | delay boundaries | referenced | macro_parser → timing_boundaries |
| §24.1 | L1670 | source and action limits | referenced | macro_parser → null_empty_and_output_arguments |
| §24.1 | L1671 | exact error offsets | referenced | macro_parser → case_whitespace_and_line_endings |
| §24.1 | L1672 | property/fuzz inputs | referenced | macro_parser → output_plan_reuse_contract |
| §24.1 | L1673 | cancellation-safe compiled plans | referenced | macro_parser → braces_and_character_policy |
| §24.2 | L1679 | create/read/update/delete | referenced | storage_sets → argument_validation |
| §24.2 | L1680 | stale revisions | referenced | storage_sets → repository_deinit_is_a_safe_noop |
| §24.2 | L1681 | short writes | referenced | storage_atomic → create_and_replace |
| §24.2 | L1682 | full filesystem, and rejection of an over-budget write with `507` | referenced | storage_macros → missing_set_and_revision_overflow |
| §24.2 | L1683 | interruption between writing `.tmp` and `rename()`, in both orders | referenced | storage_atomic → short_io_is_completed |
| §24.2 | L1684 | boot cleanup of stray `.tmp` files | referenced | storage_atomic_recovery → (file) |
| §24.2 | L1685 | corrupt JSON, including that the corrupt file is deleted and the failure reported | referenced | storage_sets → set_limit_and_stable_order |
| §24.2 | L1687 | an index naming a package file that is absent, and a package file the index omits | **UNMAPPED** | — |
| §24.2 | L1688 | macro order preserved exactly across write, reboot, export, and restore | referenced | storage_sets → measured_user_data_tracks_set_files |
...
| §24.3 | L1700 | press and release sequence | referenced | executor_execution_tests → (file) |
| §24.3 | L1701 | chords | referenced | executor_execution_tests → (file) |
| §24.3 | L1702 | delays | referenced | executor_execution_tests → (file) |
| §24.3 | L1703 | busy rejection | referenced | executor_validation_tests → engine_and_request_validation |
| §24.3 | L1704 | cancel during text | referenced | executor_execution_tests → action_order_delay_and_status_progress |
| §24.3 | L1705 | cancel during delay | referenced | executor_execution_tests → action_order_delay_and_status_progress |
| §24.3 | L1706 | disconnect and suspend | referenced | usb_keyboard → press_reports_and_waits |
| §24.3 | L1707 | timeout | referenced | executor_execution_tests → timestamps_and_current_action_track_execution |
| §24.3 | L1708 | final release-all on every terminal path | referenced | executor_terminal_tests → (file) |
| §24.4 | L1714 | authentication and logout | referenced | auth_existing_tests → derive_failures_zero_outputs |
| §24.4 | L1715 | rate limiting | referenced | auth_existing_tests → session_expiry_and_capacity |
| §24.4 | L1716 | session expiry | referenced | auth_existing_tests → sessions |
| §24.4 | L1717 | session cookie required on every route | referenced | web_request_policy → success_and_generated_request_id |
| §24.4 | L1718 | body and upload limits | referenced | web_request_policy → failure_statuses |
| §24.4 | L1719 | invalid content type | referenced | web_request_policy → failure_statuses |
| §24.4 | L1720 | path traversal | referenced | web_server_adapter_json_static → json_envelopes |
| §24.4 | L1721 | stale revisions | referenced | web_api_repository_handlers → session_json_redaction |
| §24.4 | L1722 | busy execution | referenced | web_request_policy → get_does_not_require_csrf |
...
| §24.6 | L1752 | Windows host when available | **UNMAPPED** | — |
...
| §24.6 | L1760 | user-data preservation across firmware slot switch | **UNMAPPED** | — |
