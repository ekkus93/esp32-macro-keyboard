...
| §13.2 | L805 | Firmware MUST NOT automatically format either filesystem. | referenced | web_server_adapter_json_static → static_file_selection<br>check-layer-boundaries.sh (gate script)<br>check-mount-policy.sh (gate script) |
| §13.2 | L808 | Web-assets failure MUST NOT expose an unauthenticated fallback UI. | referenced | web_server_adapter_json_static → static_file_selection<br>check-layer-boundaries.sh (gate script)<br>check-mount-policy.sh (gate script) |
| §13.3 | L822 | This is the whole tree: two paths and one object type. There MUST NOT be a | referenced | storage_mount → unmount_continues_after_one_failure<br>storage_package_restore → (file)<br>storage_sets → (file)<br>storage_sets → duplicate_index_is_discarded_and_output_cleared<br>web_api_repository_handlers → (file) |
| §13.5 | L865 | not atomic across sets**, and MUST NOT pretend to be: each set file is written | referenced | backup_restore → (file)<br>storage_package_restore → (file)<br>web_api_admin_boundary → restore_failure_is_visible |
...
| §12.2 | L744 | A macro's `source` MUST compile before it is stored. Creation and update | referenced | storage_object_json → set_document_rejects_duplicate_macro_ids<br>storage_object_json → set_document_round_trip |
| §12.3 | L765 | the index, is a corruption of the index and is handled under §13.6. Firmware MUST | referenced | app-sets → persists real settings updates<br>provisioning_settings → (file)<br>storage_active_set_delete → (file)<br>storage_sets → delete_is_permanent_and_leaves_no_trash<br>web_api_json → settings_update_matrix<br>web_api_repository_handlers → set_routes |
| §13.1 | L789 | Exact sizes are defined in `firmware/partitions.csv` and MUST be validated | gate-enforced | check-partitions.sh (gate script) |
