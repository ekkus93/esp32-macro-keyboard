...
| §17 | L1317 | `GET /api/v1/backup` MUST NOT let one damaged object make the repository | referenced | backup_restore → (file)<br>storage_package → valid_package_and_backup_documents<br>storage_package_backup → backup_output_passes_secret_sentinel_scanner<br>storage_package_backup → uncompilable_macro_is_skipped_not_fatal |
| §17 | L1337 | MUST NOT report `200` for a run that failed to write some of them. | referenced | backup_restore → (file)<br>storage_package → valid_package_and_backup_documents<br>storage_package_backup → backup_output_passes_secret_sentinel_scanner<br>storage_package_backup → uncompilable_macro_is_skipped_not_fatal |
...
| §17 | L1322 | A partial backup MUST be self-describing, so it can never be mistaken for a | referenced | backup_restore → (file)<br>storage_package → valid_package_and_backup_documents<br>storage_package_backup → backup_output_passes_secret_sentinel_scanner<br>storage_package_backup → uncompilable_macro_is_skipped_not_fatal |
| §17 | L1330 | I/O, storage unavailable, timeout) MUST still fail the export, because | referenced | backup_restore → (file)<br>storage_package → valid_package_and_backup_documents<br>storage_package_backup → backup_output_passes_secret_sentinel_scanner<br>storage_package_backup → uncompilable_macro_is_skipped_not_fatal |
...
| §24.2 | L1679 | create/read/update/delete | referenced | storage_packages → argument_validation |
| §24.2 | L1680 | stale revisions | referenced | storage_packages → repository_deinit_is_a_safe_noop |
| §24.2 | L1681 | short writes | referenced | storage_atomic → create_and_replace |
| §24.2 | L1682 | full filesystem, and rejection of an over-budget write with `507` | referenced | storage_macros → missing_package_and_revision_overflow |
...
| §24.2 | L1685 | corrupt JSON, including that the corrupt file is deleted and the failure reported | referenced | storage_packages → package_limit_and_stable_order |
