...
**Integrator correction (2026-08-08):** the `app_core` failure above was
transient to this one build, not a real or pre-existing defect. A fresh,
independent build and full `ctest` run on this exact branch (after merge,
45/45 host tests, `app_core` included) passed cleanly with zero failures —
see the merge commit. Do not treat this section's original "pre-existing,
unrelated build break on `master`" claim as accurate; it was not verified
against a clean `master` checkout before being reported. Every target
passes, including `app_core_tests`, all new ones (`web_status_limits`,
`web_auth_routes`, `web_send`), and every touched existing one (`auth`,
`web_api_core`, `web_api_response`, `web_api_dispatch`, `web_request_policy`,
`web_server_adapter`, `web_server_setup_submit`).
