...
V2-040 is **not** being marked complete in `docs/TODO_V2.md` — no
physical-hardware evidence for the setup flow itself exists yet, independent
of the note below.

**Integrator update (2026-08-08):** at the time this report was written,
V2-041's ESP32-S3R8 hardware benchmark had not yet run, so
`AUTH_PBKDF2_ITERATIONS` was still the unvalidated `120000U` placeholder and
this branch added an `AUTH_V2_PBKDF2_ITERATIONS_PROVISIONAL` alias plus the
"provisional" framing described below to make that unambiguous. The
benchmark has since run (see
`docs/implementation-v2/V2_041_PBKDF2_BENCHMARK_2026-08-08.md`) and
`AUTH_PBKDF2_ITERATIONS` is frozen at `5500U` on `master`. During merge, the
now-redundant provisional alias was removed and the one call site that used
it (`web_server_setup.c`'s `setup_password_create`) now references
`AUTH_PBKDF2_ITERATIONS` directly — the defensive fail-closed check it does
(reject if `auth_password_create()`'s default iteration count ever drifts
from what setup expects) is otherwise unchanged. The rest of this report is
left as originally written, describing the implementation at the time it was
built.
