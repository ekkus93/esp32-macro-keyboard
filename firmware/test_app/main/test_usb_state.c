...
TEST_CASE("USB keyboard initializes without emitting a key", "[device][usb]")
{
    ...
    TEST_ASSERT_TRUE(state == USB_KEYBOARD_DISCONNECTED ||
                     state == USB_KEYBOARD_ENUMERATING ||
                     state == USB_KEYBOARD_READY ||
                     state == USB_KEYBOARD_SUSPENDED);
    TEST_ASSERT_EQUAL(APP_ERROR_INVALID_ARGUMENT, usb_keyboard_press(0U, 0U));

    if (state != USB_KEYBOARD_READY) {
        TEST_ASSERT_EQUAL(APP_ERROR_USB_NOT_READY,
                          usb_keyboard_press(0U, 0x04U));
    }
}
