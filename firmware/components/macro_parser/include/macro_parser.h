...
/* Matches USB_KEYBOARD_KEYCODE_COUNT (usb_keyboard_ops.h): a HID boot keyboard
 * report carries at most this many simultaneous non-modifier key usages.
 * macro_parser has no build dependency on usb_keyboard, so this is its own
 * copy of the same number rather than a shared header. */
#define MACRO_ACTION_USAGES_MAX 6U
...
typedef struct {
    ...
    uint8_t modifiers;
    /* First usage_count entries are the simultaneous non-modifier key usages;
     * the remainder are unused. usage_count is 0 for a standalone modifier tap
     * (e.g. {CTRL} alone) and 1 for every ordinary single-key action; a
     * MACRO_ACTION_CHORD produced by a [...] group can carry up to
     * MACRO_ACTION_USAGES_MAX. */
