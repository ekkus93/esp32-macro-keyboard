...
static app_error_code_t adapter_read_device_id(void *context,
                                               uint8_t output[PROVISIONING_DEVICE_ID_BYTES]) {
    (void)context;
    return esp_read_mac(output, ESP_MAC_WIFI_SOFTAP) == ESP_OK ? APP_ERROR_NONE
                                                               : APP_ERROR_INTERNAL;
}

static app_error_code_t adapter_calculate_hmac(void *context, const uint8_t *message,
                                               size_t message_size,
                                               uint8_t output[PROVISIONING_HMAC_BYTES]) {
    (void)context;
    return esp_hmac_calculate((hmac_key_id_t)CONFIG_NVS_SEC_HMAC_EFUSE_KEY_ID, message,
