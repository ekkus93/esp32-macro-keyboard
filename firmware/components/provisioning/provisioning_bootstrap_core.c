...
static const uint8_t CODE_DOMAIN[DOMAIN_BYTES] = {
    'm', 'a', 'c', 'r', 'o', '-', 's', 'e', 't', 'u', 'p', '-', 'c', 'o', 'd', 'e', '-', 'v', '1', 0,
};
...
static void encode_hex(const uint8_t *input,
                       size_t input_size,
                       char *output,
...
static app_error_code_t derive_secret(
    ...
    const app_error_code_t result = operations->calculate_hmac(
        operations->context, message, sizeof(message), digest);
...
app_error_code_t provisioning_bootstrap_derive_with_ops(
    const provisioning_bootstrap_ops_t *operations,
