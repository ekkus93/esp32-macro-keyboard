...
static int adapter_derive(void *context, const char *password, size_t password_length,
                          const uint8_t *salt, uint32_t iterations, uint8_t *output) {
    (void)context;
    return mbedtls_pkcs5_pbkdf2_hmac_ext(MBEDTLS_MD_SHA256, (const unsigned char *)password,
                                         password_length, salt, AUTH_SALT_BYTES, iterations,
                                         AUTH_HASH_BYTES, output);
}
