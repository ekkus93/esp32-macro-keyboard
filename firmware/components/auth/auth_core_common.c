...
static bool operations_valid(const auth_ops_t *operations) {
    return operations != NULL && operations->lock != NULL && operations->unlock != NULL &&
           operations->random_fill != NULL && operations->now_us != NULL &&
           operations->derive != NULL && operations->secure_zero != NULL;
}
...
static app_error_code_t random_hex(auth_core_t *core, char *output, size_t output_size,
                                   size_t random_bytes) {
    ...
    for (size_t index = 0U; index < random_bytes; ++index) {
        ...
        output[(index * 2U) + 1U] = digits[bytes[index] & LOW_NIBBLE_MASK];
    }
    ...
    for (size_t index = 0U; index < APP_SESSION_TABLE_MAX; ++index) {
        if (core->sessions[index].active &&
            auth_core_constant_time_equal((const uint8_t *)core->sessions[index].view.session_token,
                                          (const uint8_t *)token, AUTH_TOKEN_HEX_BYTES - 1U)) {
            return true;
        }
    }
    return false;
