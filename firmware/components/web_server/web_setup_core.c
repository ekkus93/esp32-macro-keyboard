...
static bool operations_valid(const web_setup_ops_t *operations) {
    return operations != NULL && operations->provisioning_load != NULL &&
           operations->password_create != NULL &&
           operations->provisioning_commit != NULL &&
           operations->wait_for_confirmation != NULL &&
           operations->schedule_restart != NULL &&
...
static bool configuration_valid(const web_setup_configuration_t *configuration) {
    ...
    return canonical_string(configuration->device_id,
                            sizeof(configuration->device_id),
                            &device_length) &&
           device_length == PROVISIONING_DEVICE_ID_HEX_BYTES &&
           uppercase_hex(configuration->device_id, device_length) &&
           canonical_string(configuration->ap_ssid,
                            sizeof(configuration->ap_ssid),
                            &ssid_length) &&
           ssid_length > 0U &&
           canonical_string(configuration->setup_code,
...
static bool submission_valid(const web_setup_submission_t *submission,
    ...
    if (!canonical_string(submission->setup_code,
                          sizeof(submission->setup_code),
                          &code_length) ||
        code_length != PROVISIONING_SETUP_SECRET_HEX_BYTES ||
        !uppercase_hex(submission->setup_code, code_length) ||
        !canonical_string(submission->ap_ssid,
                          sizeof(submission->ap_ssid),
                          &ssid_length) ||
        ssid_length == 0U ||
        !canonical_string(submission->ap_passphrase,
        ...
        !canonical_string(submission->administrator_password,
                          ...
                          &password_length) ||
        password_length < AUTH_PASSWORD_MIN_BYTES ||
        password_length > AUTH_PASSWORD_MAX_BYTES) {
        return false;
    }
...
web_setup_state_t web_setup_core_get_state(const web_setup_core_t *core) {
    ...
    memcpy(state.device_id,
           core->configuration.device_id,
           sizeof(state.device_id));
    memcpy(state.ap_ssid,
           core->configuration.ap_ssid,
           sizeof(state.ap_ssid));
    state.completed = core->completed;
    state.physical_confirmation_required =
        core->configuration.physical_confirmation_required &&
...
static app_error_code_t authorize_submission(web_setup_core_t *core,
    ...
    if (!constant_time_equal(submission->setup_code,
                             core->configuration.setup_code,
                             PROVISIONING_SETUP_SECRET_HEX_BYTES)) {
        return APP_ERROR_AUTH_FAILED;
    }
    if (core->configuration.physical_confirmation_required &&
        !core->configuration.manufacturing_bypass) {
        return core->operations.wait_for_confirmation(
            core->operations.context, WEB_SETUP_CONFIRMATION_TIMEOUT_MS);
    }
    return APP_ERROR_NONE;
}

static provisioning_config_t replacement_configuration(
    const provisioning_config_t *current,
    const web_setup_submission_t *submission,
    ...
    memcpy(replacement.ap_ssid,
           submission->ap_ssid,
           sizeof(replacement.ap_ssid));
    memcpy(replacement.ap_passphrase,
           submission->ap_passphrase,
           sizeof(replacement.ap_passphrase));
    return replacement;
}
...
app_error_code_t web_setup_core_submit(web_setup_core_t *core,
    ...
    size_t password_length = 0U;
    if (core == NULL || !core->initialized || submission == NULL ||
        out_committed == NULL) {
        return APP_ERROR_INVALID_ARGUMENT;
    }
    ...
    if (result == APP_ERROR_NONE) {
        result = core->operations.provisioning_load(
            core->operations.context, &current);
    }
    if (result == APP_ERROR_NONE && current.provisioned) {
        result = APP_ERROR_CONFLICT;
    }
    if (result == APP_ERROR_NONE) {
        result = core->operations.password_create(
    ...
    if (result == APP_ERROR_NONE) {
        replacement = replacement_configuration(&current,
                                                submission,
                                                &password_record);
        result = core->operations.provisioning_commit(
            core->operations.context,
            &replacement,
            current.revision,
            &committed);
