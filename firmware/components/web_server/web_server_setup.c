 ...
 * AUTH_V2_PBKDF2_ITERATIONS_PROVISIONAL is not the frozen hardware-measured
 * iteration count -- see auth.h. */
static app_error_code_t setup_password_create(void *context, const char *password,
    ...
    if (record.iterations != AUTH_V2_PBKDF2_ITERATIONS_PROVISIONAL) {
