...
static app_error_code_t storage_package_list_locked(storage_package_list_t *out_list);
static app_error_code_t storage_package_create_locked(const macro_package_t *set);
static app_error_code_t storage_package_update_locked(const macro_package_t *replacement,
                                                      uint32_t expected_revision,
                                                      macro_package_t *out_updated);
static app_error_code_t storage_package_delete_locked(const app_uuid_t *set_id,
                                                      uint32_t expected_revision);
...
app_error_code_t storage_package_read(const app_uuid_t *set_id, macro_package_t *out_package) {
    ...
    const app_error_code_t result = storage_package_read_locked(set_id, out_package);
...
app_error_code_t storage_package_list(storage_package_list_t *out_list) {
    ...
    const app_error_code_t result = storage_package_list_locked(out_list);
    const app_error_code_t unlock = storage_repository_lock_give();
    return unlock == APP_ERROR_NONE ? result : APP_ERROR_INTERNAL;
}
...
app_error_code_t storage_package_create(const macro_package_t *set) {
    ...
    const app_error_code_t result = storage_package_create_locked(set);
    const app_error_code_t unlock = storage_repository_lock_give();
    return unlock == APP_ERROR_NONE ? result : APP_ERROR_INTERNAL;
}
...
app_error_code_t storage_package_update(const macro_package_t *replacement,
    ...
    const app_error_code_t result =
        storage_package_update_locked(replacement, expected_revision, out_updated);
    const app_error_code_t unlock = storage_repository_lock_give();
    return unlock == APP_ERROR_NONE ? result : APP_ERROR_INTERNAL;
}
...
app_error_code_t storage_package_delete(const app_uuid_t *set_id, uint32_t expected_revision) {
    ...
    const app_error_code_t result = storage_package_delete_locked(set_id, expected_revision);
    const app_error_code_t unlock = storage_repository_lock_give();
    return unlock == APP_ERROR_NONE ? result : APP_ERROR_INTERNAL;
}
...
app_error_code_t storage_package_read_locked(const app_uuid_t *set_id,
    ...
    if (set_id == NULL || out_package == NULL) {
        return APP_ERROR_INVALID_ARGUMENT;
    }
    storage_package_document_t document = {0};
    const app_error_code_t result = storage_repository_load_package_document(set_id, &document);
    if (result == APP_ERROR_NONE) {
        *out_package = document.set;
    }
...
static app_error_code_t storage_package_list_locked(storage_package_list_t *out_list) {
    ...
    for (size_t item = 0U; item < index.count; ++item) {
        result = storage_package_read_locked(&index.ids[item], &out_list->items[item]);
        if (result != APP_ERROR_NONE) {
            memset(out_list, 0, sizeof(*out_list));
            return result;
        }
...
static app_error_code_t prepare_package_create(const macro_package_t *set,
                                               storage_package_index_t *index) {
    app_error_code_t result = storage_repository_load_index(index);
    if (result != APP_ERROR_NONE) {
        return result;
    }
    ...
    result = storage_make_package_path(&set->id, path, sizeof(path));
    if (result != APP_ERROR_NONE) {
        return result;
    }
...
static app_error_code_t storage_package_create_locked(const macro_package_t *set) {
    ...
    storage_package_index_t index = {0};
    app_error_code_t result = prepare_package_create(set, &index);
    if (result != APP_ERROR_NONE) {
        return result;
    }
    ...
    if (result != APP_ERROR_NONE) {
        const app_error_code_t cleanup = storage_repository_remove_package_file(&set->id);
...
static app_error_code_t storage_package_update_locked(const macro_package_t *replacement,
    ...
    storage_package_document_t document = {0};
    app_error_code_t result = storage_repository_load_package_document(&replacement->id, &document);
    if (result != APP_ERROR_NONE) {
        return result;
    }
...
static app_error_code_t storage_package_delete_locked(const app_uuid_t *set_id,
    ...
    app_error_code_t result = storage_package_read_locked(set_id, &current);
    if (result != APP_ERROR_NONE) {
        return result;
    }
    ...
    if (index.has_active_package && app_uuid_equal(&index.active_package_id, set_id)) {
        index.has_active_package = false;
        memset(&index.active_package_id, 0, sizeof(index.active_package_id));
    }
    ...
    return storage_repository_remove_package_file(set_id);
}

static app_error_code_t storage_package_select_locked(const app_uuid_t *set_id) {
    ...
    app_error_code_t result = storage_package_read_locked(set_id, &set);
    if (result != APP_ERROR_NONE) {
        return result;
    }
    storage_package_index_t index = {0};
    ...
    index.has_active_package = true;
    index.active_package_id = *set_id;
...
app_error_code_t storage_package_select(const app_uuid_t *set_id) {
    ...
    const app_error_code_t result = storage_package_select_locked(set_id);
    const app_error_code_t unlock = storage_repository_lock_give();
    return unlock == APP_ERROR_NONE ? result : APP_ERROR_INTERNAL;
}
...
app_error_code_t storage_active_package_read(bool *out_has_active_package,
    ...
    if (out_has_active_package == NULL || out_package_id == NULL) {
        return APP_ERROR_INVALID_ARGUMENT;
    }
    *out_has_active_package = false;
    memset(out_package_id, 0, sizeof(*out_package_id));
    const app_error_code_t lock = storage_repository_lock_take();
    if (lock != APP_ERROR_NONE) {
        return lock;
    }
    ...
    if (result == APP_ERROR_NONE) {
        *out_has_active_package = index.has_active_package;
        *out_package_id = index.active_package_id;
    }
    const app_error_code_t unlock = storage_repository_lock_give();
    return unlock == APP_ERROR_NONE ? result : APP_ERROR_INTERNAL;
}
