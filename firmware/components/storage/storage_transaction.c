...
static app_error_code_t production_set_index_presence(void *context, const app_uuid_t *set_id,
...
static app_error_code_t manifest_id_from_path(const char *path, app_uuid_t *out_id) {
    static const char prefix[] = STORAGE_DATA_MOUNT "/transactions/";
    if (out_id != NULL) {
...
static bool manifest_strings_terminated(const storage_transaction_manifest_t *manifest) {
...
static bool manifest_revisions_valid(const storage_transaction_manifest_t *manifest) {
...
static bool manifest_shape_valid(const storage_transaction_manifest_t *manifest) {
...
app_error_code_t storage_transaction_write_manifest_with_ops(
    ...
    if (!manifest_shape_valid(manifest) || !storage_fs_ops_is_valid(operations) ||
        generate_uuid == NULL) {
        return APP_ERROR_INVALID_ARGUMENT;
    }
    ...
    return storage_atomic_write_with_ops(path, manifest, sizeof(*manifest), true, operations,
                                         generate_uuid, uuid_context);
}

app_error_code_t
storage_transaction_write_manifest(const storage_transaction_manifest_t *manifest) {
    return storage_transaction_write_manifest_with_ops(manifest, storage_fs_ops_posix(),
...
static app_error_code_t parse_set_id_from_path(const char *path, const char *prefix,
                                               app_uuid_t *out_id) {
...
static app_error_code_t remove_manifest_with_ops(const storage_transaction_manifest_t *manifest,
                                                 const storage_fs_ops_t *operations) {
    char path[APP_PATH_MAX_BYTES];
    const app_error_code_t result = manifest_path(&manifest->id, path, sizeof(path));
    if (result != APP_ERROR_NONE) {
...
static app_error_code_t write_recovery_manifest(const storage_transaction_manifest_t *manifest,
                                                const storage_fs_ops_t *operations,
                                                storage_uuid_generate_fn generate_uuid,
                                                void *uuid_context) {
...
static app_error_code_t recover_create(storage_transaction_manifest_t *manifest,
    ...
    const int staging_length = snprintf(expected_staging, sizeof(expected_staging),
                                        STORAGE_DATA_MOUNT "/staging/%s", manifest->id.value);
    if (staging_length < 0 || (size_t)staging_length >= sizeof(expected_staging) ||
        strcmp(manifest->staging, expected_staging) != 0 || manifest->source[0] != '\0' ||
        manifest->backup[0] != '\0') {
        return APP_ERROR_STORAGE_CORRUPT;
    }
    ...
    app_uuid_t set_id = {0};
    app_error_code_t result =
        parse_set_id_from_path(manifest->destination, STORAGE_DATA_MOUNT "/sets/", &set_id);
    if (result != APP_ERROR_NONE) {
        return result;
    }
...
static app_error_code_t recover_delete(storage_transaction_manifest_t *manifest,
    ...
    app_uuid_t set_id = {0};
    app_error_code_t result =
        parse_set_id_from_path(manifest->source, STORAGE_DATA_MOUNT "/sets/", &set_id);
    if (result != APP_ERROR_NONE) {
        return result;
    }
    char expected_backup[APP_PATH_MAX_BYTES];
    const int backup_length =
        snprintf(expected_backup, sizeof(expected_backup), STORAGE_DATA_MOUNT "/trash/%s-%s",
                 set_id.value, manifest->id.value);
    if (backup_length < 0 || (size_t)backup_length >= sizeof(expected_backup) ||
...
static app_error_code_t collect_manifest_paths_with_ops(char paths[][APP_PATH_MAX_BYTES],
                                                        size_t *out_count,
                                                        const storage_fs_ops_t *operations) {
    if (out_count != NULL) {
        *out_count = 0U;
    }
    if (paths == NULL || out_count == NULL || !storage_fs_ops_has_directory(operations)) {
        return APP_ERROR_INVALID_ARGUMENT;
    }
    void *directory =
    ...
    while (true) {
        ...
        const int written =
            snprintf(paths[count], APP_PATH_MAX_BYTES, STORAGE_DATA_MOUNT "/transactions/%s", name);
...
static app_error_code_t directory_has_entries_with_ops(const char *path,
                                                       const storage_fs_ops_t *operations,
...
app_error_code_t storage_transaction_recover_all_with_ops(
    ...
    size_t count = 0U;
    app_error_code_t result = collect_manifest_paths_with_ops(paths, &count, operations);
    if (result != APP_ERROR_NONE) {
        return result;
    }
    ...
    for (size_t index = 0U; index < count; ++index) {
        ...
        if (result == APP_ERROR_NONE) {
            ...
            case STORAGE_TRANSACTION_IMPORT_PACKAGE_SET:
                ...
                break;
            case STORAGE_TRANSACTION_DELETE_SET:
                result = recover_delete(&manifest, operations, generate_uuid, uuid_context,
                                        set_index_presence, index_context);
                break;
            case STORAGE_TRANSACTION_REPLACE_SET:
    ...
    result = directory_has_entries_with_ops(STORAGE_DATA_MOUNT "/staging", operations,
                                            &staging_has_entries);
    if (result != APP_ERROR_NONE && first_error == APP_ERROR_NONE) {
        first_error = result;
    }
