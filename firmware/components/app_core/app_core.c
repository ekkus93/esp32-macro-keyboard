...
static app_error_code_t adapter_settings_init(void *context) {
    ...
    const app_error_code_t recovery_result = factory_reset_recovery_run_if_pending(&recovered);
    if (recovery_result != APP_ERROR_NONE) {
        ESP_LOGE(TAG, "factory reset recovery failed before ordinary startup: %s",
                 app_error_code_string(recovery_result));
        return recovery_result;
    }
    if (recovered) {
        ESP_LOGW(TAG, "factory reset recovery completed; continuing into unprovisioned setup");
    }
