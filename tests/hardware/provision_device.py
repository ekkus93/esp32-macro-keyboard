...
This helper is also used by reset acceptance checks. An unprovisioned device
never starts the USB stack, so it does not enumerate as a keyboard until setup
has completed.

Credentials are generated rather than typed. They are disposable bench secrets,
stored outside the repository by ``hil_state`` and never printed.
...
def send_confirmation() -> None:
    """Give physical confirmation over the UART console."""
...
def main() -> int:
    ...
    setup_code = (hil_state.state_dir() / "setup_code.txt").read_text(
        encoding="utf-8"
    ).strip()
    ...
    while time.time() < deadline:
        ...
        try:
            ...
            continue
    print(
        f"error: device did not come back provisioned within {RESTART_TIMEOUT_S}s",
        file=sys.stderr,
    )
    return 1
