#!/usr/bin/env python3
"""Send every macro in a package, in its stored order, and read back the wire.

    SPEC 24.6 item: a full package of macros sent in order against a harmless text target

The other typing test sends one macro at a time and checks each in isolation.
This one checks the property that only shows up across a whole package: that the
...
Usage:
    python3 tests/hardware/test_full_package_in_order.py
"""
...
SET_NAME = "Ordered acceptance package"
...
def build_set(device):
    """Create a package whose macros are stored in a known order."""
    ...
    status, _ = device.post(f"/api/v1/package/{package_id}/select", {})
    if status != 200:
        raise SystemExit(f"error: could not select the package: {status}")
...
def run_macro(device, package_id, macro_id, timeout_s=20):
    status, payload = device.post(
        "/api/v1/executions",
        {"packageId": package_id, "macroId": macro_id, "macroRevision": 1},
    )
    if status != 202:
...
def main() -> int:
    ...
    device.login()
    try:
        package_id, created = build_set(device)
        print(f"created package {package_id} with {len(created)} macros")
        ...
        if stored != expected_order:
            raise SystemExit(
                f"error: the package came back in a different order than it was stored\n"
        ...
        with Capture() as capture:
            for index, macro_id in enumerate(stored, 1):
                state = run_macro(device, package_id, macro_id)
                if state != "completed":
        ...
        if typed != expected_text:
            raise SystemExit("error: the wire does not match the package's stored order")
        if not released:
            raise SystemExit("error: a key was still held after the last macro")
        print("\nPASS: the whole package typed in stored order, nothing left held")
        return 0
    finally:
        device.logout()
