...
uint64_t fake_clock_now_us(fake_clock_t *clock) {
    ...
    return fake_call_log_record(&clock->calls, "clock_now_us", clock->now_us, 0U) ? UINT64_MAX
                                                                                  : clock->now_us;
}
...
uint32_t fake_clock_now_ms(fake_clock_t *clock) {
    ...
    return fake_call_log_record(&clock->calls, "clock_now_ms", now_ms, 0U) ? UINT32_MAX : now_ms;
}
