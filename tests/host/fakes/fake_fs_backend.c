...
static const char *operation_name(fake_fs_operation_t operation) {
    static const char *const names[FAKE_FS_OPERATION_COUNT] = {
        "fs_open",        "fs_read",     "fs_write",     "fs_close", "fs_sync",
        "fs_sync_parent", "fs_rename",   "fs_unlink",    "fs_stat",  "fs_mkdir",
        "fs_open_dir",    "fs_read_dir", "fs_close_dir", "fs_rmdir",
    };
...
static bool should_fail(fake_fs_backend_t *filesystem, fake_fs_operation_t operation) {
    ...
    if (fake_call_log_record(&filesystem->calls, operation_name(operation),
                             filesystem->operation_counts[index], 0U)) {
        errno = EIO;
        return true;
    }
    for (size_t failure_index = 0U; failure_index < filesystem->failure_count; ++failure_index) {
        const fake_fs_failure_t *failure = &filesystem->failures[failure_index];
...
void fake_fs_backend_add_failure(fake_fs_backend_t *filesystem, fake_fs_operation_t operation,
                                 size_t occurrence, int error_number) {
    if (filesystem == NULL || operation < 0 || operation >= FAKE_FS_OPERATION_COUNT ||
        occurrence == 0U || error_number == 0 ||
        filesystem->failure_count >= FAKE_FS_FAILURE_CAPACITY) {
        abort();
    }
    filesystem->failures[filesystem->failure_count++] = (fake_fs_failure_t){
        .operation = operation,
...
void fake_fs_backend_fail_on(fake_fs_backend_t *filesystem, fake_fs_operation_t operation,
                             size_t occurrence, int error_number) {
    if (filesystem == NULL) {
        abort();
    }
    ...
    fake_fs_backend_add_failure(filesystem, operation, occurrence, error_number);
}
...
ssize_t fake_fs_read(fake_fs_backend_t *filesystem, int descriptor, void *buffer, size_t length) {
...
ssize_t fake_fs_write(fake_fs_backend_t *filesystem, int descriptor, const void *buffer,
                      size_t length) {
...
int fake_fs_read_dir(fake_fs_backend_t *filesystem, void *directory, char *name, size_t name_size,
                     bool *out_end) {
