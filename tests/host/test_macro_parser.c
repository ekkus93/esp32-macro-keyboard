...
static void expect_success(const char *source, size_t expected_actions)
{
    ...
    CHECK(result == APP_ERROR_NONE);
    CHECK(plan.action_count == expected_actions);
...
{
    app_uuid_t uuid = {0};
    CHECK(app_uuid_parse("123e4567-e89b-42d3-a456-426614174000", &uuid) == APP_ERROR_NONE);
    CHECK(app_uuid_is_valid_string(uuid.value));
    CHECK(!app_uuid_is_valid_string("123e4567-e89b-12d3-a456-426614174000"));
    CHECK(!app_uuid_is_valid_string("../bad"));
    CHECK(macro_model_validate_revision(0U) == APP_ERROR_INVALID_ARGUMENT);
    CHECK(macro_model_validate_revision(1U) == APP_ERROR_NONE);
}
...
static void test_fuzz_corpus(void)
{
    ...
    for (size_t iteration = 0U; iteration < 10000U; ++iteration) {
        ...
        if (result == APP_ERROR_NONE) {
            CHECK(plan.action_count <= APP_COMPILED_ACTION_MAX);
            macro_plan_free(&plan);
        } else {
            CHECK(plan.actions == NULL);
            CHECK(plan.action_count == 0U);
...
static void test_printable_ascii(void)
{
    for (int value = 0x20; value <= 0x7e; ++value) {
        ...
        CHECK(macro_keymap_us_printable((char)value, &key));
        CHECK(key.usage != 0U);
    }
}
