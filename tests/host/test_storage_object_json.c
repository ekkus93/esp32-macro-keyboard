...
static void test_package_document_round_trip(void) {
    ...
    const storage_package_document_t input = {
    ...
    storage_package_document_free(&output);
    cJSON_free(json);
}

/* A set file that repeats a macro id is corrupt: nothing downstream can address
 * either copy unambiguously. */
static void test_package_document_rejects_duplicate_macro_ids(void) {
    ...
    TEST_CHECK_APP_ERROR(
        APP_ERROR_STORAGE_CORRUPT,
        storage_package_document_parse(duplicated, sizeof(duplicated) - 1U, &output));
    storage_package_document_free(&output);
}

/* A stored macro that carries set_id is not a stored macro, it is a package
 * entry in the wrong container (SPEC 12.2). */
static void test_package_document_rejects_stored_macro_with_package_id(void) {
    static const char with_package_id[] =
        "{\"schema_version\":1,\"id\":\"00000001-0000-4000-8000-000000000003\","
        "\"revision\":1,\"name\":\"Dup\",\"macros\":["
        "{\"schema_version\":1,\"id\":\"00000001-0000-4000-8000-000000000009\","
    ...
    TEST_CHECK_APP_ERROR(
        APP_ERROR_STORAGE_CORRUPT,
        storage_package_document_parse(with_package_id, sizeof(with_package_id) - 1U, &output));
    storage_package_document_free(&output);
}
...
int main(void) {
    ...
    test_package_document_round_trip();
    test_package_document_rejects_duplicate_macro_ids();
    test_package_document_rejects_stored_macro_with_package_id();
