...
static macro_package_t make_package(uint32_t value, const char *name) {
...
static void assert_package_layout(const macro_package_t *set) {
    TEST_CHECK(set != NULL);
...
static void rewrite_package_file(const app_uuid_t *path_id, const macro_package_t *contents) {
    TEST_CHECK(path_id != NULL);
    TEST_CHECK(contents != NULL);
    char path[APP_PATH_MAX_BYTES];
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_make_package_path(path_id, path, sizeof(path)));
    char *json = NULL;
    size_t json_length = 0U;
    const storage_package_document_t document = {.set = *contents};
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE,
                         storage_package_document_serialize(&document, &json, &json_length));
...
static void test_argument_validation(void) {
    reset_store();
    storage_package_list_t list = {0};
    macro_package_t output = {0};
    macro_package_t set = make_package(1U, "Arguments");

    TEST_CHECK_APP_ERROR(APP_ERROR_INVALID_ARGUMENT, storage_package_list(NULL));
    TEST_CHECK_APP_ERROR(APP_ERROR_INVALID_ARGUMENT, storage_package_read(NULL, &output));
    TEST_CHECK_APP_ERROR(APP_ERROR_INVALID_ARGUMENT, storage_package_read(&set.id, NULL));
    TEST_CHECK_APP_ERROR(APP_ERROR_INVALID_ARGUMENT, storage_package_create(NULL));
    TEST_CHECK_APP_ERROR(APP_ERROR_INVALID_ARGUMENT, storage_package_update(NULL, 1U, &output));
    TEST_CHECK_APP_ERROR(APP_ERROR_INVALID_ARGUMENT, storage_package_update(&set, 0U, &output));
    TEST_CHECK_APP_ERROR(APP_ERROR_INVALID_ARGUMENT, storage_package_update(&set, 1U, NULL));
    TEST_CHECK_APP_ERROR(APP_ERROR_INVALID_ARGUMENT, storage_package_delete(NULL, 1U));
    TEST_CHECK_APP_ERROR(APP_ERROR_INVALID_ARGUMENT, storage_package_delete(&set.id, 0U));

    set.revision = 2U;
    TEST_CHECK_APP_ERROR(APP_ERROR_INVALID_ARGUMENT, storage_package_create(&set));
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_list(&list));
...
static void test_crud_ordering_revisions_and_cleanup(void) {
    reset_store();
    macro_package_t first = make_package(10U, "First");
    macro_package_t second = make_package(20U, "Second");
    macro_package_t third = make_package(30U, "Third");

    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_create(&first));
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_create(&second));
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_create(&third));
    assert_package_layout(&first);
    assert_package_layout(&second);
    assert_package_layout(&third);

    TEST_CHECK_APP_ERROR(APP_ERROR_CONFLICT, storage_package_create(&second));
    ...
    macro_package_t replacement = first;
    TEST_CHECK(snprintf(replacement.name, sizeof(replacement.name), "Updated First") > 0);
    macro_package_t updated = {0};
    TEST_CHECK_APP_ERROR(APP_ERROR_CONFLICT, storage_package_update(&replacement, 2U, &updated));
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_update(&replacement, 1U, &updated));
    TEST_CHECK_EQ_U64(2U, updated.revision);
    TEST_CHECK_EQ_STRING("Updated First", updated.name);

    macro_package_t readback = {0};
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_read(&first.id, &readback));
    ...
    TEST_CHECK_APP_ERROR(APP_ERROR_CONFLICT, storage_package_update(&replacement, 1U, &readback));
    TEST_CHECK_APP_ERROR(APP_ERROR_CONFLICT, storage_package_delete(&second.id, 2U));
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_delete(&second.id, 1U));

    char deleted_path[APP_PATH_MAX_BYTES];
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE,
                         storage_make_package_path(&second.id, deleted_path, sizeof(deleted_path)));
    ...
    TEST_CHECK_APP_ERROR(APP_ERROR_NOT_FOUND, storage_package_read(&second.id, &readback));
    TEST_CHECK_APP_ERROR(APP_ERROR_NOT_FOUND, storage_package_delete(&second.id, 1U));
}

static void test_revision_overflow_is_rejected(void) {
    reset_store();
    macro_package_t set = make_package(40U, "Maximum revision");
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_create(&set));

    set.revision = UINT32_MAX;
    rewrite_package_file(&set.id, &set);

    macro_package_t output = {0};
    TEST_CHECK_APP_ERROR(APP_ERROR_CONFLICT, storage_package_update(&set, UINT32_MAX, &output));
...
static void test_package_limit_and_stable_order(void) {
    reset_store();
    for (uint32_t index = 0U; index < APP_MACRO_SETS_MAX; ++index) {
        ...
        macro_package_t set = make_package(1000U + index, name);
        TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_create(&set));
    }

    macro_package_t extra = make_package(9000U, "One too many");
    TEST_CHECK_APP_ERROR(APP_ERROR_STORAGE_FULL, storage_package_create(&extra));
...
static void test_corrupt_package_is_discarded(void) {
    reset_store();
    macro_package_t set = make_package(50U, "Corrupt JSON");
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_create(&set));
...
static void test_mismatched_object_id_is_discarded(void) {
    reset_store();
    macro_package_t expected = make_package(60U, "Expected");
    macro_package_t wrong = make_package(61U, "Wrong ID");
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_create(&expected));
    rewrite_package_file(&expected.id, &wrong);

    char path[APP_PATH_MAX_BYTES];
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE,
                         storage_make_package_path(&expected.id, path, sizeof(path)));
    macro_package_t output;
    memset(&output, 0xa5, sizeof(output));
    TEST_CHECK_APP_ERROR(APP_ERROR_STORAGE_CORRUPT, storage_package_read(&expected.id, &output));
    TEST_CHECK_EQ_U64(0U, output.revision);
...
static void test_duplicate_index_is_discarded_and_output_cleared(void) {
    reset_store();
    macro_package_t set = make_package(70U, "Duplicate index");
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_create(&set));
    ...
    storage_package_list_t list;
    memset(&list, 0xa5, sizeof(list));
    TEST_CHECK_APP_ERROR(APP_ERROR_STORAGE_CORRUPT, storage_package_list(&list));
    TEST_CHECK_EQ_U64(0U, list.count);
...
static void test_create_leaves_no_staging_artifacts(void) {
    reset_store();
    macro_package_t set = make_package(80U, "Direct Create");
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_create(&set));

    char destination[APP_PATH_MAX_BYTES];
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE,
                         storage_make_package_path(&set.id, destination, sizeof(destination)));
...
static void test_delete_is_permanent_and_leaves_no_trash(void) {
    reset_store();
    macro_package_t set = make_package(90U, "Permanent Delete");
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_create(&set));

    char source[APP_PATH_MAX_BYTES];
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE,
                         storage_make_package_path(&set.id, source, sizeof(source)));
...
static void test_index_is_not_rebuilt_from_the_packages_directory(void) {
    reset_store();
    macro_package_t set = make_package(120U, "Orphaned by index loss");
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_create(&set));
...
static app_error_code_t interloper_delete(void) {
    return storage_package_delete(&g_interloper_id, 1U);
}
...
static void test_concurrency_same_revision_updates_conflict(void) {
    reset_store();
    macro_package_t set = make_package(1U, "Serialize");
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_create(&set));

    macro_package_t first = set;
    first.revision = 1U;
    macro_package_t updated = {0};
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_update(&first, 1U, &updated));
    TEST_CHECK_EQ_U64(2U, updated.revision);

    macro_package_t second = set;
    second.revision = 1U;
    macro_package_t updated_again = {0};
    TEST_CHECK_APP_ERROR(APP_ERROR_CONFLICT, storage_package_update(&second, 1U, &updated_again));
}
...
static void test_concurrency_create_excludes_delete(void) {
    reset_store();
    macro_package_t existing = make_package(1U, "Existing");
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_create(&existing));
    g_interloper_id = existing.id;

    macro_package_t created = make_package(2U, "Created");
    g_mx = (mx_lock_state_t){.interloper = interloper_delete, .interloper_armed = true};
    storage_repository_lock_set_ops(&mx_ops);
    const app_error_code_t result = storage_package_create(&created);
...
static void test_concurrency_lock_failures_are_visible(void) {
    reset_store();
    macro_package_t set = make_package(1U, "LockFail");
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_create(&set));
    ...
    first.revision = 1U;
    macro_package_t updated = {0};
    const app_error_code_t give_failure = storage_package_update(&first, 1U, &updated);
    storage_repository_lock_set_ops(NULL);
    ...
    second.revision = 2U;
    macro_package_t updated_again = {0};
    const app_error_code_t take_failure = storage_package_update(&second, 2U, &updated_again);
    storage_repository_lock_set_ops(NULL);
    ...
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_read(&set.id, &current));
    TEST_CHECK_EQ_U64(2U, current.revision);
}

/* Measured use counts the bytes actually on disk. */
static void test_measured_user_data_tracks_package_files(void) {
    reset_store();
    ...
    macro_package_t set = make_package(210U, "Measured");
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_create(&set));
    size_t with_package = 0U;
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_repository_measure_user_data(NULL, &with_package));
    TEST_CHECK(with_package > 0U);
 ...
 *
 * storage_package_reorder had no test at all before this. These are written from
...
static void test_reorder_preserves_the_requested_order_exactly(void) {
    reset_store();
    macro_package_t a = make_package(300U, "Alpha");
    macro_package_t b = make_package(301U, "Bravo");
    macro_package_t c = make_package(302U, "Charlie");
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_create(&a));
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_create(&b));
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_create(&c));
    ...
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_reorder(requested, 3U));
...
static void test_reorder_rejects_anything_but_a_permutation(void) {
    reset_store();
    macro_package_t a = make_package(310U, "Alpha");
    macro_package_t b = make_package(311U, "Bravo");
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_create(&a));
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_create(&b));
    ...
    TEST_CHECK_APP_ERROR(APP_ERROR_CONFLICT, storage_package_reorder(missing_one, 1U));
    TEST_CHECK_APP_ERROR(APP_ERROR_CONFLICT, storage_package_reorder(repeated, 2U));
    TEST_CHECK_APP_ERROR(APP_ERROR_CONFLICT, storage_package_reorder(unknown, 2U));
...
static void test_reorder_does_not_disturb_the_active_package(void) {
    reset_store();
    macro_package_t a = make_package(320U, "Alpha");
    macro_package_t b = make_package(321U, "Bravo");
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_create(&a));
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_create(&b));
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_select(&b.id));

    const app_uuid_t swapped[] = {b.id, a.id};
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_reorder(swapped, 2U));
...
int main(void) {
    ...
    test_reorder_does_not_disturb_the_active_package();
    test_measured_user_data_tracks_package_files();
    test_repository_deinit_is_a_safe_noop();
    test_crud_ordering_revisions_and_cleanup();
    test_revision_overflow_is_rejected();
    test_package_limit_and_stable_order();
    test_corrupt_package_is_discarded();
    ...
    test_index_is_not_rebuilt_from_the_packages_directory();
