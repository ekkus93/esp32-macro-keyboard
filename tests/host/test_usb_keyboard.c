...
static bool fake_send_report(void *context, uint8_t report_id, uint8_t modifiers,
...
#define CHECK_MISSING(member)                                                                      \
    do {                                                                                           \
        ...
        TEST_CHECK(!usb_keyboard_ops_is_valid(&operations));                                       \
        TEST_CHECK_EQ_INT(APP_ERROR_INVALID_ARGUMENT, usb_keyboard_state_init(&operations));       \
    } while (0)
...
static void test_press_rejects_invalid_states(void) {
    static const usb_keyboard_state_t rejected_states[] = {
        USB_KEYBOARD_UNINITIALIZED, USB_KEYBOARD_DISCONNECTED, USB_KEYBOARD_ENUMERATING,
        USB_KEYBOARD_SUSPENDED,     USB_KEYBOARD_ERROR,
    };
    for (size_t index = 0U; index < (sizeof(rejected_states) / sizeof(rejected_states[0]));
         ++index) {
        usb_fixture_t fixture;
        reset_fixture(&fixture);
...
static void test_release_all(void) {
    ...
    reset_fixture(&fixture);
    usb_keyboard_ops_t operations = make_operations(&fixture);
    TEST_CHECK_EQ_INT(APP_ERROR_USB_NOT_READY, usb_keyboard_state_release_all(&operations));

    ready_fixture(&fixture);
    TEST_CHECK_EQ_INT(APP_ERROR_NONE, usb_keyboard_state_release_all(&operations));
    TEST_CHECK_EQ_U64(1U, fixture.report_calls);
    TEST_CHECK_EQ_INT(0U, fixture.report_id);
