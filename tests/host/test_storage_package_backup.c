...
static void test_backup_contains_complete_repository_deterministically(void) {
    ...
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_export_backup(&second, &second_length));
...
static void test_backup_output_passes_secret_sentinel_scanner(void) {
     ...
     * storage_package_export_backup - which only ever reads set and macro
     * repository data - cannot leak a real secret that
    ...
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE, storage_package_export_backup(&output, &output_length));
...
static void test_failure_preserves_primary_error_and_cleans_partial_snapshot(void) {
    ...
    char *data = (char *)1;
    size_t length = 99U;
    TEST_CHECK_APP_ERROR(APP_ERROR_IO, storage_package_export_backup(&data, &length));
    TEST_CHECK(data == NULL);
