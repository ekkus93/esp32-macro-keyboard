...
static app_error_code_t fake_read_blob(void *context,
                                       uint8_t *output,
                                       size_t capacity,
                                       size_t *out_size) {
    ...
    if (fake->read_error != APP_ERROR_NONE &&
        (fake->read_error_occurrence == 0U ||
         fake->read_calls == fake->read_error_occurrence)) {
...
static app_error_code_t fake_write_blob(void *context,
                                        const uint8_t *data,
                                        size_t size) {
    fake_provisioning_t *fake = context;
    ++fake->write_calls;
...
static provisioning_config_t valid_configuration(uint32_t revision) {
    ...
    TEST_CHECK_EQ_INT(36,
                      snprintf(configuration.active_set_id.value,
                               sizeof(configuration.active_set_id.value),
                               "%s",
...
static void start_empty(provisioning_core_t *core, fake_provisioning_t *fake) {
    ...
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE,
                         provisioning_core_load(core, &loaded));
    TEST_CHECK(!loaded.provisioned);
    TEST_CHECK_EQ_U64(0U, loaded.revision);
}

static void commit_valid(provisioning_core_t *core,
...
static void test_init_and_default_load(void) {
    ...
    TEST_CHECK_APP_ERROR(APP_ERROR_INVALID_ARGUMENT,
                         provisioning_core_init(NULL, &operations));
    TEST_CHECK_APP_ERROR(APP_ERROR_INVALID_ARGUMENT,
                         provisioning_core_init(&core, NULL));
    ...
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE,
                         provisioning_core_init(&core, &operations));
    TEST_CHECK_APP_ERROR(APP_ERROR_INVALID_ARGUMENT,
                         provisioning_core_load(NULL, NULL));
    TEST_CHECK_APP_ERROR(APP_ERROR_INVALID_ARGUMENT,
                         provisioning_core_load(&core, NULL));
    provisioning_config_t loaded;
    ...
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE,
                         provisioning_core_deinit(&core));
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE,
                         provisioning_core_deinit(&core));
    TEST_CHECK_APP_ERROR(APP_ERROR_INVALID_ARGUMENT,
                         provisioning_core_deinit(NULL));
}
...
static void test_configuration_validation(void) {
    ...
    TEST_CHECK(!provisioning_config_is_valid(&configuration));
    configuration = valid_configuration(0U);
    memset(configuration.ap_passphrase,
           'x',
           sizeof(configuration.ap_passphrase));
    TEST_CHECK(!provisioning_config_is_valid(&configuration));
    configuration = valid_configuration(0U);
...
static void test_corrupt_persisted_records(void) {
    ...
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE,
                         provisioning_core_init(&reader, &operations));
    TEST_CHECK_APP_ERROR(APP_ERROR_STORAGE_CORRUPT,
                         provisioning_core_load(&reader, &loaded));
    fake.durable_size += 1U;
...
static void test_clear_credentials(void) {
    ...
    commit_valid(&core, &fake, &committed);
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE,
                         provisioning_core_clear_credentials(&core));
    provisioning_config_t loaded;
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE,
                         provisioning_core_load(&core, &loaded));
    TEST_CHECK(!loaded.provisioned);
...
static void test_factory_reset(void) {
    ...
    TEST_CHECK_APP_ERROR(APP_ERROR_IO,
                         provisioning_core_factory_reset(&core));
    TEST_CHECK(fake.present);
