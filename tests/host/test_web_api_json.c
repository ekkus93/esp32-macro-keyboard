...
static void test_expected_revision_and_mutation(void) {
    ...
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE,
                         web_api_json_parse_expected_revision(delete_body,
                                                              sizeof(delete_body) - 1U,
                                                              &revision));
    ...
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE,
                         web_api_json_parse_resource_mutation(update, sizeof(update) - 1U,
                                                              512U, &mutation));
...
static void test_order_and_execution(void) {
    ...
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE,
                         web_api_json_parse_uuid_order(order, sizeof(order) - 1U, 4U,
                                                       &parsed));
    ...
    TEST_CHECK_APP_ERROR(APP_ERROR_INVALID_ARGUMENT,
                         web_api_json_parse_uuid_order(duplicate, sizeof(duplicate) - 1U, 4U,
                                                       &parsed));
    ...
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE,
                         web_api_json_parse_execution_submit(execution,
                                                             sizeof(execution) - 1U,
                                                             &request));
    TEST_CHECK(request.has_procedure_context);
    TEST_CHECK_EQ_U64(7U, request.macro_revision);

    const char partial_context[] =
        "{\"setId\":\"" SET_ID "\",\"macroId\":\"" MACRO_ID
        "\",\"macroRevision\":7,\"procedureId\":\"" PROCEDURE_ID "\"}";
    TEST_CHECK_APP_ERROR(APP_ERROR_INVALID_ARGUMENT,
                         web_api_json_parse_execution_submit(partial_context,
                                                             sizeof(partial_context) - 1U,
                                                             &request));
}
...
static void test_progress_and_settings(void) {
    ...
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE,
                         web_api_json_parse_progress_action(complete,
                                                            sizeof(complete) - 1U,
                                                            false, &action));
    ...
    const char skip_unconfirmed[] =
        ...
        "\",\"confirmed\":false}";
    TEST_CHECK_APP_ERROR(APP_ERROR_INVALID_ARGUMENT,
                         web_api_json_parse_progress_action(skip_unconfirmed,
                                                            sizeof(skip_unconfirmed) - 1U,
                                                            true, &action));
    ...
    uint32_t revision = 0U;
    TEST_CHECK_APP_ERROR(APP_ERROR_NONE,
                         web_api_json_parse_settings_update(settings,
                                                            sizeof(settings) - 1U,
                                                            &parsed, &revision));
...
static void test_trailing_and_embedded_nul_rejected(void) {
    ...
    TEST_CHECK_APP_ERROR(APP_ERROR_INVALID_ARGUMENT,
                         web_api_json_parse_expected_revision(trailing,
                                                              sizeof(trailing) - 1U,
                                                              &revision));
    const char nul_escape[] = "{\"expectedRevision\":1,\"x\":\"\\u0000\"}";
    TEST_CHECK_APP_ERROR(APP_ERROR_INVALID_ARGUMENT,
                         web_api_json_parse_expected_revision(nul_escape,
                                                              sizeof(nul_escape) - 1U,
                                                              &revision));
}
