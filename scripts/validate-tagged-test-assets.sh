...
# Build the manifest in a temp file outside the artifact tree so the running
# find never observes the manifest being written, then move it into place.
manifest="$(mktemp)"
(
	cd -- "${artifact_dir}"
...
mv -- "${manifest}" "${artifact_dir}/MANIFEST.sha256"
