...
describe("H4 execution recovery", () => {
  test(
    "persistent tracking failure becomes explicit and GET-only retry resumes tracking",
    async () => {
      vi.useFakeTimers();
      try {
  ...
  test(
    "409 plus failed recovery blocks a second POST until GET reconciliation",
    async () => {
      const onStatus = vi.fn();
      planJsonResponse(
        {
  ...
  test(
    "overlay keeps Retry and Cancel visible and reports cancel delivery failure",
    async () => {
      planJsonResponse(
        {
          error: {
